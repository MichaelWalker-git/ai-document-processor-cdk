...
export class BackendAppStack extends cdk.Stack {
  ...
  private addMarketplaceNagSuppressions(
    ...
    const bucketNotificationHandlerPath = this.node.findAll().find(child =>
      child.node.id.includes('BucketNotificationsHandler'),
    );

    if (bucketNotificationHandlerPath) {
      NagSuppressions.addResourceSuppressions(
        [bucketNotificationHandlerPath],
        [
          {
            id: 'AwsSolutions-IAM4',
            reason: 'BucketNotificationsHandler requires AWSLambdaBasicExecutionRole for CloudWatch access',
            appliesTo: ['Policy::arn:<AWS::Partition>:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole'],
          },
          {
            id: 'AwsSolutions-IAM5',
            reason: 'BucketNotificationsHandler requires wildcard permissions for S3 notification management',
            appliesTo: ['Resource::*'],
          },
          {
            id: 'HIPAA.Security-IAMNoInlinePolicy',
            reason: 'CDK-generated inline policies for bucket notification management',
          },
        ],
        true,
      );
